...
def run_text2img_experiments() -> None:
    model_id = DEFAULT_MODEL_ID
    seed = 42
    prompt = "ultra-realistic product photo of a luxury perfume bottle on a white background, studio lighting, soft shadow, very sharp, crystal glass"
    negative = "text, watermark, logo, low quality, blurry, deformed"
...
def run_img2img_experiments() -> None:
    ...
    init_path = "inputs/produit.jpg"

    prompt = "luxury headset with gold accents and a microphone, professional product photography, studio lighting"
    ...
    for name, strength in strengths:
        g = make_generator(seed, device)  # Nouveau générateur pour chaque run
